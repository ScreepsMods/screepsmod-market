  ...
  let ps = terminals.map(async terminal => {
    ...
    Object.keys(market.buyPrices).forEach(mineral => {
      ...
      if(market.priceMode === 'fixed') {
        ...
        buy.remainingAmount = Math.floor(market.buyAmount * Math.random() * 2)
        buy.totalAmount = Math.floor(buy.remainingAmount * Math.random())
        buy.amount = Math.floor(buy.totalAmount * Math.random())
    ...
    Object.keys(market.sellPrices).forEach(mineral => {
      ...
      if(market.priceMode === 'fixed') {
        ...
        sell.remainingAmount = Math.floor(market.sellAmount * Math.random() * 2)
        sell.totalAmount = Math.floor(sell.remainingAmount * Math.random())
        sell.amount = Math.floor(sell.totalAmount * Math.random())
